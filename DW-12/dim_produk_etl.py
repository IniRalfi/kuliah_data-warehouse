...
def load_dim_produk(df: pd.DataFrame):
    ...
    try:
        ...
        print(
            f"Berhasil memuat {len(df)} baris data produk ke tabel 'dim_produk' di DW PostgreSQL."
        )
    except Exception as e:
...
@flow(
    name="ETL Flow: Dimensi Produk",
    description="Pipeline khusus untuk mengisi dim_produk dari tb_products",
)
