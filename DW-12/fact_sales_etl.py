        ...
        return pd.DataFrame(
            [
                ...
                },
                {
                    ...
                    "tgl_transaksi": "2024-10-16",
                    "customer_id": "C002",
                    "store_id": "ST-02",
                    "product_id": "PROD-003",
                    "qty": 5,
                    "subtotal_harga": 500000,
                },
            ]
        )
...
def extract_dim_lookups() -> dict:
    ...
    try:
        ...
        df_pelanggan = pd.read_sql(
            "SELECT sk_pelanggan, customer_id FROM dim_pelanggan", conn
        )
...
def transform_fact_sales(df_sales: pd.DataFrame, dim_lookups: dict) -> pd.DataFrame:
    ...
    df_sales["sk_waktu_transaksi"] = (
        df_sales["tgl_transaksi"].dt.strftime("%Y%m%d").astype(int)
    )
...
@flow(
    name="ETL Flow: Fact Sales",
    description="Pipeline tabel fakta dengan proses Lookup silang database",
)
