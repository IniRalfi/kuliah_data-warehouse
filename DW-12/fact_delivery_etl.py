            ...
            [
                {
                    ...
                    "kurir": "Khatulistiwa Cargo",
                    "status_pengiriman": "DELIVERED",
                    "tgl_sampai": "2024-10-11T14:30:00Z",
                    "biaya_ongkir": 25000,
                    "durasi_hari": 1,
                },
                {
                    ...
                    "kurir": "J&T",
                    "status_pengiriman": "DELIVERED",
                    "tgl_sampai": "2024-10-12T14:30:00Z",
                    "biaya_ongkir": 40000,
                    "durasi_hari": 3,
                },
                {
...
def extract_dw_lookups() -> dict:
    ...
    try:
        ...
        dim_dict["kurir"] = pd.read_sql(
            "SELECT sk_kurir, nama_kurir FROM dim_kurir", conn
        )
        conn.close()
    except Exception as e:
        print("Gagal akses DW. Menggunakan dummy...")
        dim_dict["toko"] = pd.DataFrame(
            {
                "sk_toko": [1, 2, 3],
                "store_id": ["STR-PTK-01", "STR-PTK-02", "STR-STG-01"],
            }
        )
        dim_dict["kurir"] = pd.DataFrame(
...
def transform_fact_delivery(
    df_api: pd.DataFrame, df_oltp: pd.DataFrame, dim_lookups: dict
) -> pd.DataFrame:
    ...
    dim_lookups["toko"]["store_id"] = (
        dim_lookups["toko"]["store_id"].astype(str).str.upper()
    )
    dim_lookups["kurir"]["nama_kurir"] = (
        dim_lookups["kurir"]["nama_kurir"].astype(str).str.title()
    )
    ...
    # Dapatkan sk_kurir
    df = df.merge(
        dim_lookups["kurir"], left_on="kurir", right_on="nama_kurir", how="left"
    )
...
@flow(
    name="ETL Flow: Fact Delivery",
    description="Pipeline kompleks menggabungkan API, OLTP, dan DW",
)
