            ...
            [
                {
                    ...
                    "Bulan": 1,
                    "ID_Toko": "STR-PTK-01",
                    "Target_Pendapatan_Rp": 150000000,
                },
                {
                    "Tahun": 2024,
                    "Bulan": 1,
                    "ID_Toko": "STR-PTK-02",
                    "Target_Pendapatan_Rp": 120000000,
                },
                {
                    "Tahun": 2024,
                    "Bulan": 1,
                    "ID_Toko": "STR-STG-01",
                    "Target_Pendapatan_Rp": 85000000,
                },
            ]
        )
...
def transform_fact_target_sales(
    df_csv: pd.DataFrame, df_toko: pd.DataFrame
) -> pd.DataFrame:
    ...
    df_csv["tanggal_str"] = (
        df_csv["Tahun"].astype(str)
        + "-"
        + df_csv["Bulan"].astype(str).str.zfill(2)
        + "-01"
    )
    ...
    df_merged = df_csv.merge(
        df_toko, left_on="ID_Toko", right_on="store_id", how="left"
    )
...
@flow(
    name="ETL Flow: Fact Target Sales",
    description="Pipeline target penjualan dari file CSV",
)
